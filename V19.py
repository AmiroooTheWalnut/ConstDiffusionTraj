...
class SimpleNN(nn.Module):
    def __init__(self, forbiddenSerialMap=None, lenForbidden=10, maxLengthSize = 10, temporalFeatureSize=2, convOffset=0, batchSize=-1):
        ...
        self.psl = psl
        self.pslSum = pslSum
        # self.TRE = nn.TransformerEncoderLayer(d_model=self.size, nhead=4)
        ...
        self.denseAfterCat4Traj = nn.Linear(self.temporalFeatureSize * 6, self.size)
        # self.normDenseAfterCat4Traj = nn.LayerNorm([self.maxLengthSize,self.size])
        self.normDenseAfterCat4Traj = nn.LayerNorm([self.maxLengthSize, self.temporalFeatureSize*6])
        self.denseAfterCat4Forb = nn.Linear(self.temporalFeatureSize * 6, self.size)
        # self.normDenseAfterCat4Forb = nn.LayerNorm([self.maxLengthSize,self.size])
        self.normDenseAfterCat4Forb = nn.LayerNorm([self.maxLengthSize, self.temporalFeatureSize*6])
        ...
        self.lastAdd = Add3()
        # self.lastNormLayer = nn.LayerNorm([self.maxLengthSize, self.size + 50])
    ...
    def forward(self, traj, time, condLat, condLon):
        ...
        mergePathTraj = self.normDenseAfterCat4Traj(convedTrajs)
        mergePathTraj2 = self.denseAfterCat4Traj(mergePathTraj)
        # mergePathTraj2 = self.normDenseAfterCat4Traj(mergePathTraj)

        mergePathForb = self.normDenseAfterCat4Forb(convedDists)
        mergePathForb2 = self.denseAfterCat4Forb(mergePathForb)
        # mergePathForb2 = self.normDenseAfterCat4Forb(mergePathForb)

        allInfo = torch.cat((mergePathTraj2,mergePathForb2,timePath),dim=2)
        ...
        finalPath = torch.cat((timePath, allInfoDrnsed, trajPathDirect,mergePathTraj2,mergePathForb2), 2)
        ...
        # finalPathC2 = self.lastNormLayer(finalPathC)
        finalPathC3 = self.lastDense(finalPathC)
        # finalPathC = self.activationFinal(finalPathC)

        # finalPathC = finalPathC.permute(0, 2, 1)
        # finalPathC = self.convLast(finalPathC)
        # finalPathC = finalPathC.permute(0, 2, 1)
        finalPathC4 = self.lastDenseAlt(finalPathC3)
        return finalPathC4
...
def generate_ts(timesteps, num, learningScheduleTime, isChangeWeights, isAdvancedWeighting=True):
    ...
    lastOnes = np.random.randint((int)(math.floor(timesteps * 0.7)), timesteps,
                                 size=(int)(num * 0.03))
    lastOnes1 = np.random.randint((int)(math.floor(timesteps * 0.6)), timesteps,
                                  size=(int)(num*0.05))
...
def forward_noise_notNormalized(meanVals, varVals, timesteps, x, t, learningScheduleTime, isChangeWeights, isVizualize=True, isAdvancedWeighting=True):
    ...
    if isAdvancedWeighting == True:
        ...
        img_a = x * (1 - np.pow(a, 2 + learningScheduleTime * 2.0)) + noise * np.pow(a, 2 + learningScheduleTime * 2.0)
        img_b = x * (1 - np.pow(b, 2 + learningScheduleTime * 2.0)) + noise * np.pow(b, 2 + learningScheduleTime * 2.0)
    else:
...
class ConvLayers(nn.Module):
    def __init__(self, maxLengthSize,size,lenOffest,featureSize, convOffset=0):
        ...
        self.TRE = nn.TransformerEncoderLayer(d_model=self.size, nhead=4)
        self.TRED1 = nn.Linear(self.size, maxLengthSize)
        self.TRED2 = nn.Linear(self.maxLengthSize, self.featureSize)
        self.TREConv1 = nn.Conv1d(self.maxLengthSize+lenOffest, self.maxLengthSize, 3, padding=1)
    ...
    def forward(self,input):
        inNorm = self.beforeTRE(input)
        tre = self.TRE(inNorm)
        treConv1 = self.TREConv1(tre)
        treD1 = self.TRED1(treConv1)
        treD2 = self.TRED2(treD1)
        lstm = self.LSTM(input)[0]
        lSTMConv1 = self.LSTMConv1(lstm)
        lstmD1 = self.LSTMD1(lSTMConv1)
        # lstmD2 = self.LSTMD2(lstmD1)
        ...
        mergePath3Total = torch.cat((mergePath1, mergePath2, mergePath3, xd42, treD2, lstmD1), 2)
